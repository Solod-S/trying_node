...
// 1) проверяет валидность токена (лже или протухший токен не допустим)
// 2) извлекает из токена id, находит пользователя по id в базе и прикрепляет его к запросу (req.user)
//
...
const auth = async (req, res, next) => {
  ...
  // извлекаем загловок авторизации, если его нет то по умолчанию пустая строка (нам не нужен undefined)
  ...
  try {
    ...
    // у нас в jwt.sign зашит payload ={}
